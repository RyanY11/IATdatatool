    ...
    for i in part_list:
        ...
        if ratio > 0.01*percent:
            if type == 'fast':
                ...
                add_line = pd.DataFrame.from_dict({'受试者编号': i, '剔除原因': '过快反应占比高于设定值', '详情': text})
            else:
                text = '过慢反应试次数： ' + str(count) + '占比： ' + str(100*ratio) + '%'
                add_line = pd.DataFrame.from_dict({'受试者编号': i, '剔除原因': '过慢反应占比高于设定值', '详情': text})
